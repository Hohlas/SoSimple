...
| [api_server.py](api_server.py) | REST API (FastAPI) для приёма фракталов от MT4; использует общий live-safe preprocessing перед inference | HTTP request → ML prediction | 🔬 |
