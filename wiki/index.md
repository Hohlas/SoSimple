...
| [fractal-stop-research.md](research/fractal-stop-research.md) | Stage 1—6.3 + Regression Up/Dn target line: breach, fav, exit, walk-forward, Transformer, A7-аудит, profile screening, time-to-breach reformulation, price/ATR ablation, outcome-based triple-barrier, H12/H6 geometry and price-action parity check, `range_w1_atr` post-mortem, target foundation, ratio audit, already-moved audit, next-open entry target foundation, entry-based price-feature matrix, fractal selection ablation, closeout, powerful tabular capacity check, ordered sequence Transformer, amplitude movement-regime audit и bounded movement filter. Итог последнего подцикла: top-level `up_*/dn_*` содержат сильный сигнал от `fractal0_price`, но схема входа `next open after signal_time` не прошла direction даже после сильных табличных моделей и текущей ограниченной sequence-матрицы; amplitude / movement-regime связь сильная, а поверх неё найден только простой research-only movement filter `simple_combined / extra_trees_small / H3 / top 5%`. `time_plus_atr` остаётся почти столь же сильным baseline; direction, PnL/PF и `locked_test` по-прежнему запрещены. | 43 report updates (06-10 — 07-07) |
