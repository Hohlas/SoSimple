...
| [fractal-stop-research.md](research/fractal-stop-research.md) | Stage 1—6.3 + Regression Up/Dn target line: breach, fav, exit, walk-forward, Transformer, A7-аудит, profile screening, time-to-breach reformulation, price/ATR ablation, outcome-based triple-barrier, H12/H6 geometry and price-action parity check, `range_w1_atr` post-mortem, target foundation, ratio audit, already-moved audit, next-open entry target foundation, entry-based price-feature matrix, fractal selection ablation, closeout, powerful tabular capacity check, ordered sequence Transformer, amplitude movement-regime audit, bounded movement filter, freeze-репликация, repaired direction-inside-mask reject, rich-features full grid, narrow seed-stability replication, `fractal0_price` retest-zone oracle и полный Fractal0 entry/exit grid. Итог последнего подцикла: entry/exit grid дал `val_eval PF=1.9438`, `BS_p05=1.7601`, stress PF `1.5743` и permutation PASS; post-review M5 winner-only пересчёт снял искусственный ambiguity cap для ML-exit winner (`ambiguous_same_bar_rate=0.0`), но нужен полный rerun или frozen subset с M5 execution contract. | 50 report updates (06-10 — 07-21) |
