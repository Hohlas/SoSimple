...
| [fractal-stop-research.md](research/fractal-stop-research.md) | Stage 1—6.3 + Regression Up/Dn target line: breach, fav, exit, walk-forward, Transformer, A7-аудит, profile screening, time-to-breach reformulation, price/ATR ablation, outcome-based triple-barrier, H12/H6 geometry and price-action parity check, `range_w1_atr` post-mortem, target foundation, ratio audit, already-moved audit, next-open entry target foundation, entry-based price-feature matrix, fractal selection ablation, closeout, powerful tabular capacity check, ordered sequence Transformer, amplitude movement-regime audit, bounded movement filter, freeze-репликация, repaired direction-inside-mask reject, rich-features full grid, narrow seed-stability replication, `fractal0_price` retest-zone oracle, полный Fractal0 entry/exit grid, stop-policy grid и entry-quality filter. Итог последнего подцикла: после аудита `entry_quality_top10` для `S2/E3/M0/X2` выбран на `val_select`, но провалился на `val_eval` (`BS_p05=0.9713` против no-mask `2.2865`); lifecycle `research_hint`, `locked_test` не открыт. | 52 report updates (06-10 — 07-21) |
