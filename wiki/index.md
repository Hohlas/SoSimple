...
| [fractal-stop-research.md](research/fractal-stop-research.md) | Fractal Stop Stage 1-5.1b: breach, fav, exit, walk-forward, Transformer, A7-аудит, `asinh`, XGBoost+Logistic скрининг, проверка переобучения, диагностика устойчивости сигнала, structural field ablation и Up/Dn field ablation. Stage 5.1b: `back` остаётся главным устойчивым полем после добавления `shift`; Up/Dn дают слабый самостоятельный сигнал, но не улучшают `structure_full`; модельные Up/Dn нормализованы per-pair, delta CI отсутствуют; ветка `H6_off05` остаётся `DIAGNOSTIC_ONLY`. | 24 report updates (06-10 — 06-25) |
