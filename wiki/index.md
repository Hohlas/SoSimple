...
| [fractal-stop-research.md](research/fractal-stop-research.md) | Stage 1—6.3 + Regression Up/Dn target line: breach, fav, exit, walk-forward, Transformer, A7-аудит, profile screening, time-to-breach reformulation, price/ATR ablation, outcome-based triple-barrier, H12/H6 geometry and price-action parity check, `range_w1_atr` post-mortem, target foundation, ratio audit, already-moved audit, next-open entry target foundation и entry-based price-feature matrix. Итог последнего подцикла: top-level `up_*/dn_*` содержат сильный сигнал от `fractal0_price`, но схема входа `next open after signal_time` не переоткрыта ни пересчётом target-а от фактического `entry_open`, ни ограниченной матрицей ценовых признаков. | 37 report updates (06-10 — 07-02) |
