...
| [fractal-stop-research.md](research/fractal-stop-research.md) | Stage 1—6.3 + Regression Up/Dn target line: breach, fav, exit, walk-forward, Transformer, A7-аудит, profile screening, time-to-breach reformulation, price/ATR ablation, outcome-based triple-barrier, H12/H6 geometry and price-action parity check, `range_w1_atr` post-mortem, target foundation, ratio audit, already-moved audit, next-open entry target foundation, entry-based price-feature matrix, fractal selection ablation, closeout, powerful tabular capacity check, ordered sequence Transformer, amplitude movement-regime audit, bounded movement filter, freeze-репликация, repaired direction-inside-mask reject, rich-features full grid, narrow seed-stability replication и `fractal0_price` retest-zone oracle. Итог последнего подцикла: `fractal0_price` retest-zone дал diagnostic ratio, но gate не прошёл из-за `active_years=2` при требовании `3`; verdict `DIAGNOSTIC_ONLY`, следующий шаг только frozen probe-plan. | 49 report updates (06-10 — 07-10) |
