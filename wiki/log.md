...
- 2026-07-27: Updated fixed11 pruning policy to choose representatives inside strong-duplicate groups by `BS_p05 / max_drawdown_r`; refreshed report/wiki wording and retained subset.
- 2026-07-27: Ingested 2026-07-27-fractal0-fixed11-retained-subset-mt4-parity.md; feasibility stopped at `UNKNOWN / parity_blocked` because one plain `time;signal` stream cannot preserve 5 retained rules with duplicate/opposite `signal_time`, and current MT4 direct mode is not proven to execute Python fixed11 `E3/S2/X2/spread=0.20`.
