...
39. **Stage 6.2: H12 price action даёт слабый диагностический сигнал, но не additive edge.** Primary `h12_price_action_core` прошёл AUC/PF/trades/spread checks (val AUC `0.6233`, selected PF `1.307`), но не прошёл permutation (`p=0.150 > 0.10`). Combined price-action profiles подняли same-run baseline AUC только примерно на `+0.010`, ниже required `+0.020`, и тоже провалили permutation (`p=0.205`). Итог — **TRADING_GATE_FAILED**, `DIAGNOSTIC_ONLY`. Вывод узкий: отвергнута фиксированная OHLC-window family, а не все price-action представления.

**Все этапы (Stage 2→6.2) отклонены как торговые кандидаты.** Табличные модели достигли потолка, Transformer не дал устойчивого улучшения, а диагностика устойчивости сигнала во времени не дала жёсткого решения. Stage 5.1/5.1b уточнили носитель структурного сигнала, Stage 5.2/5.3 проверили time-to-breach постановки и нашли полезную дискретную цель `fast`, Stage 5.4 не нашёл признаков, которые усиливают этот след до кандидата, а Stage 6.0-6.2 не превратили outcome-based H6/H12 постановки в устойчивое торговое правило. Ветка остаётся диагностической.
