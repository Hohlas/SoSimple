...
- идея времени до пробоя не закрыта на oracle-уровне;
- текущая обычная регрессия одного числа `bars_to_breach` не извлекла полезного ранжирования;
...
34. **Stage 5.2: время до пробоя имеет oracle-потолок, но текущая регрессия провалилась.** Oracle-time PF положительный на sell (`1.6520`) и buy (`1.7244`), поэтому идея использовать время до пробоя не опровергнута механически. Но XGBoost-регрессия на 7 профилях не извлекла ранжирование: Spearman `0.0000`, AUC `0.5000`, все профили равны, а MAE хуже constant baseline примерно в 3 раза. Это не candidate и не winner; это сигнал к post-mortem цели/предсказаний и к censored/ordinal постановке, если ветку продолжать.
...
- **Почему Stage 5.2 дал одинаковые нулевые метрики для всех профилей?** Нужно проверить распределения `y_pred` по profile/seed/target. Если модель схлопнулась в почти постоянное предсказание, следующая постановка должна учитывать цензуру: `breach_after_k`, ordinal buckets или survival-style вместо обычной регрессии одного числа.
...
- [2026-06-25-stage5_2-time-to-breach-regression.md](../../docs/reports/2026-06-25-stage5_2-time-to-breach-regression.md) — Stage 5.2: регрессия времени до пробоя; oracle проходит, model gate провален, все профили дают Spearman `0` и AUC `0.5`
