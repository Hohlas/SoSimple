...
58. **Fixed11 mutual-correlation pruning reduced 11 rules to 5 retained rules without new winner selection.** Read-only pruning consumed only saved fixed11 locked-test CSV/JSON and candidate-audit JSON. It returned `overall_decision=pruning_passed`, `input_rule_count=11`, `retained_count=5`, `removed_count=6`, `pair_count=55`; pairwise verdict counts are `strong_duplicate=13`, `partial_overlap=42`, `unclear_or_complementary=0`. Representatives are selected by `lowest_original_rank_then_rule_id`, not by `locked_test` PF/PnL/drawdown/`BS_p05`. Retained rules: `rank01_time_only_linear_target_entry_ev_regression_top30`, `rank05_time_only_linear_target_entry_avoid_sl_top30`, `rank07_movement_plus_time_linear_target_entry_good_0_5r_top40`, `rank09_time_only_hist_gradient_boosting_target_entry_good_0_5r_top50`, `rank10_movement_plus_time_linear_target_entry_ev_regression_top50`. Next active step is MT4/tester parity for those 5 rules only.
...
- [2026-07-27-fractal0-fixed11-mutual-correlation-pruning.md](../../docs/reports/2026-07-27-fractal0-fixed11-mutual-correlation-pruning.md) — read-only mutual-correlation pruning of 11 passed fixed rules: `pruning_passed`, retained subset 5 rules, 6 direct strong duplicate drops; next step is MT4/tester parity for retained subset
