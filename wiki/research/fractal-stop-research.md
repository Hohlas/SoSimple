...
- oracle gate формально прошёл: sell `oracle_time_pf = 1.6520`, buy `1.7244`;
- caveat: `oracle_binary_pf = inf`, `pf_delta_vs_binary = None`, поэтому сравнение time-oracle vs binary-oracle невалидно;
- oracle frequency слишком высокая: `998-1103` trades/year;
- model gate провален на обеих целях;
- все 7 профилей дали одинаковые медианные метрики;
- validation Spearman = `0.0000`, AUC `true>=4` = `0.5000` на sell и buy;
- MAE модели хуже constant baseline:
  - sell: model `4.5561` vs constant `1.4439`;
  - buy: model `4.5671` vs constant `1.4329`.

Итог Stage 5.2:

- идея времени до пробоя не закрыта, но текущий oracle gate слаб и требует исправления;
- текущий результат ненадёжен как исследовательский вывод: идентичные метрики всех профилей указывают на вероятный баг или схлопывание предсказаний;
- массивы предсказаний в JSON не сохранены, поэтому post-mortem требует перезапуска;
- `time_only` не является winner, он выбран только из-за tie-break среди одинаковых нулевых метрик;
- следующий шаг, если продолжать, — post-mortem схлопывания предсказаний и постановка с учётом цензуры (`breach_after_k`, ordinal buckets или survival-style), а не новый широкий перебор по `H6_off05`.
...
34. **Stage 5.2: артефакт времени до пробоя требует технического post-mortem.** Oracle-time PF положительный на sell (`1.6520`) и buy (`1.7244`), но binary-oracle имеет `PF = inf`, поэтому сравнение oracle-time vs oracle-binary невалидно. XGBoost-регрессия на 7 профилях дала идентичные метрики: Spearman `0.0000`, AUC `0.5000`, MAE хуже constant baseline примерно в 3 раза. Это не candidate, не winner и не надёжное опровержение идеи; это сигнал к перезапуску с сохранением предсказаний, проверкой feature/model contract и censored/ordinal постановкой, если ветку продолжать.

**Все этапы (Stage 2→5.2) отклонены как торговые кандидаты.** Табличные модели достигли потолка, Transformer не дал устойчивого улучшения, а диагностика устойчивости сигнала во времени не дала жёсткого решения. Stage 5.1/5.1b уточнили носитель структурного сигнала, Stage 5.2 проверил альтернативный target времени до пробоя, но ни один этап не переоткрыл `H6_off05`. Ветка остаётся в диагностическом неопределённом статусе.
...
- **Почему Stage 5.2 дал одинаковые нулевые метрики для всех профилей?** Текущий JSON не сохранил `y_pred`, поэтому нужен перезапуск. Проверить распределения `y_pred`, variance feature matrices, отличие профилей, variance предсказаний и сравнение с dummy regressor. Если модель схлопнулась в почти постоянное предсказание, следующая постановка должна учитывать цензуру: `breach_after_k`, ordinal buckets или survival-style вместо обычной регрессии одного числа.
...
- [2026-06-25-stage5_2-time-to-breach-regression.md](../../docs/reports/2026-06-25-stage5_2-time-to-breach-regression.md) — Stage 5.2: регрессия времени до пробоя; artifact требует post-mortem, oracle-binary PF бесконечен, model gate провален, все профили дают Spearman `0` и AUC `0.5`
