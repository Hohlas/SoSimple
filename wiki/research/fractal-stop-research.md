...
57. **Fixed11 candidate audit blocked status escalation despite strong locked-test metrics.** Locked-test OOS evaluation opened `locked_test` once for 11 frozen normalized rules and reported all 11 passing PF/BS/sample-size gates (`PF=2.6747-3.3667`, verdict `candidate_check_required`). The independent audit then returned `candidate_audit_blocked`: `20` findings, `18` errors, `2` warnings. Main blockers are missing pre-open freeze/policy artifacts, incomplete `split_roles` boundaries and validation roles, missing no-selection disclosure, unclassified low-N edge-year rows, missing `correlation_pruning_status=FOLLOW_UP_REQUIRED`, and incomplete `movement_plus_time` score restoration disclosure. Result: no mutual-correlation pruning, MT4/tester parity, stress-spread pass/fail or trading-status discussion until reproducibility/disclosure blockers are fixed without changing frozen candidate rules.
...
- [2026-07-25-fractal0-fixed11-candidate-audit.md](../../docs/reports/2026-07-25-fractal0-fixed11-candidate-audit.md) — independent read-only audit of fixed11 locked-test artifacts: `candidate_audit_blocked`, 18 errors and 2 warnings; status escalation and parity/pruning are blocked
