...
- `impulse` выглядит потенциально интересным (`add_impulse` положителен на обеих целях), но не собирает достаточно согласованности и остаётся `mixed_or_unclear`
...
- структурный сигнал не сводится к одному календарю;
...
32. **Stage 5.1: внутри structural-профиля устойчиво выделяется только `back`.** `structure_full` уверенно превосходит `time_only` на обеих целях, значит структурный сигнал не сводится к календарю. При этом только поле `back` даёт согласованный рисунок `drop-one hurts / add-one helps` на sell и buy и получает `likely_useful`. Остальные поля остаются `mixed_or_unclear`, полей `likely_noise` не найдено. Это узкий диагностический вывод, а не новое открытие кандидата.
...
- **Заслуживает ли `back` отдельного узкого follow-up?** Stage 5.1 выделил `back` как единственное `likely_useful` поле. Если по `H6_off05` вообще делать ещё один диагностический шаг, он должен быть узким и вращаться вокруг `back`, а не вокруг новой широкой сетки профилей.
- **Что делать с `impulse`?** Поле выглядит вторым по интересу, но Stage 5.1 не дал достаточной согласованности для `likely_useful`. Нужен ли ему follow-up, неясно.
...
- [2026-06-24-stage5_1-structural-field-ablation.md](../../docs/reports/2026-06-24-stage5_1-structural-field-ablation.md) — Stage 5.1: structural field ablation, `back` = единственное `likely_useful` поле
