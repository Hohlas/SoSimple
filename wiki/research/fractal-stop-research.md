...
45. **Итог по ветке Regression Up/Dn теперь жёстче, чем после target foundation.** Target family `up_*/dn_*` подтверждена как содержательный объект для движения от `fractal0_price`, но схема `next open after signal_time` для неё диагностически отклонена. Следующий допустимый шаг — только entry-механика, привязанная к `fractal0_price` или её ретесту, либо новый target, измеряемый прямо от `entry_open`.
...
- **Какой entry-механике вообще соответствует сигнал `Regression Up/Dn`?** Уже ясно, что `next open after signal_time` не работает. Открытым остаётся только узкий вопрос про вход, привязанный к `fractal0_price`: `retest-entry`, `limit-entry`, касание зоны или новый target, измеряемый прямо от `entry_open`.
