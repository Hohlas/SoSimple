...
59. **Fixed11 retained-subset MT4 parity feasibility is blocked.** The feasibility gate for `docs/reports/2026-07-27-fractal0-fixed11-retained-subset-mt4-parity.md` checked the 5 retained rules against current MT4 `iSignal=3` runtime and stopped before export. Retained trades: `6177`; unique `signal_time`: `2806`; unique `signal_time+direction`: `2827`; duplicate `signal_time` groups: `1670`; opposite-direction time groups: `21`. One plain `time;signal` file would drop `rule_id` and collapse duplicate/opposite rows, while current MT4 direct mode is not proven to execute Python fixed11 `E3/S2/X2/spread=0.20`. Verdict: `UNKNOWN`, stage decision: `parity_blocked`. No export, MT4 tester run or reconciliation was produced. Next route must be per-rule MT4 export, a new `rule_id`-preserving MT4 runtime, or a lower-status aggregate diagnostic.

60. **Fixed11 retained rules now have an MT4 per-rule switch, but parity is still open.** `$o$imple.mq4` exposes `ML_RuleSlot`; `MAIN.mqh` includes it in the numeric `#.csv` contract; `lib_ML_Signal.mqh` maps slots `1..5` to `ml_signals_fixed11_rule01.csv` ... `ml_signals_fixed11_rule05.csv`; `MT/MQL4/Files/#.csv` and `MT/tester/files/#.csv` now contain five rows selectable by `BackTest=2..6`. Per-rule signal files exist in both MT4 file directories for manual tester runs. This does not prove parity: current `time;signal` export collapses same-direction duplicate times and omits opposite-direction same-time groups because one timestamp cannot represent two directions.
...
- [2026-07-27-fractal0-fixed11-retained-subset-mt4-parity.md](../../docs/reports/2026-07-27-fractal0-fixed11-retained-subset-mt4-parity.md) — retained-subset MT4 parity feasibility: `UNKNOWN / parity_blocked`; one plain `time;signal` stream cannot preserve 5-rule identity because retained trades contain duplicate/opposite `signal_time`; no export/tester/reconciliation produced
- MT4 retained-rule switch update — `ML_RuleSlot` and `BackTest=1..5` now select one retained rule per tester run; compile/tester/reconciliation still pending
