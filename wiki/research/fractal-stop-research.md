...
- **Какой entry-механике вообще соответствует сигнал `Regression Up/Dn`?** Уже ясно, что `next open after signal_time` не работает ни при старом target от `fractal0_price`, ни при новом target от фактического `entry_open`. Открытым остаётся только узкий вопрос про вход, привязанный к `fractal0_price`: `retest-entry`, `limit-entry`, касание зоны или другой target, измеряемый прямо от фактического исполнения.
