...
- **Как лучше поставить time-to-breach после rerun?** Обычная MAE-регрессия проигрывает constant baseline, но Spearman/AUC показывают ранжирование. Кандидаты: `survives_at_least_k`, `breach_after_k`, ordinal buckets `fast / medium / no breach`.
