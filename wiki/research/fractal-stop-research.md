...
> Фрактальные признаки предсказывают пробой уровня, oracle (проверка потолка) показывает высокий диагностический потолок механики, но RF/XGBoost/Transformer пока не дают устойчивого торгового или модельного превосходства. Последний подцикл перешёл к execution-aware `fractal0_price` mechanics: stop-policy grid выбрал `S2_fractal0_buffer_0_5_entry_floor_2 / E3_open_pullback_1_0atr / M0_no_mask / X2_ml_opposite_any_p0_50`; узкий entry-quality filter провалился. Rich-entry audit исправил потерю `fractal0..fractal99`; normalized rerun исправил raw price-like scale contract, но winner остался `time_only`. Fixed11 locked-test дал сильные PF/BS numbers для 11 frozen rules и статус `candidate_check_required`; independent candidate audit вернул `candidate_audit_passed`. Mutual-correlation pruning завершён: retained subset содержит 5 правил, представители внутри дублей выбраны по `BS_p05 / max_drawdown_r`. MT4/tester parity для retained subset продвинут до per-rule diagnostic route, но fill-chronology audit показал ошибку Python execution contract. Chronology-fix убрал same-H1 `ML_CLOSE`, но уничтожил старый edge (`PF max=0.938880`, `kept_candidates=0`), поэтому fixed11 positive locked-test chain invalidated. Следующий инженерный шаг перенесён в MT5 execution-loop prototype: `$o$imple.mq5` компилируется, добавлены entry-only signal schema/export, event parser и диагностический scorer, но MT5 `Nero.csv` parity и tester metrics ещё не получены.
...
`empirical_p_value=0.004975`, `200` repeats, method
`block_shuffled_val_select_pnl_r`. Post-review добавил M5
`execution_ohlc_path` и исправил ambiguity-семантику: для ML-exit не
считается гипотетический fixed TP `0.7R`. Winner-only M5 artifact сохранил
PF/BS числа и дал `ambiguous_same_bar_rate=0.0`; val_eval
`effective_profit_years=1.9864` по формуле концентрации прибыли. Old
...
### 2026-07-21: Fractal0 entry-quality filter

Entry-quality follow-up добавил отдельный bounded runner поверх stop-grid
winner без копирования симулятора. Проверялась только E3 mechanics:
`S2/E3/M0/X2_ml_opposite_any_p0_50`. Entry labels строились на фактических
train_core сделках: `target_entry_good = pnl_r > 0` и
`target_entry_avoid_sl = close_reason != "SL"`. После аудита признаки
переведены на pre-order planned contract: `side_buy`, `ATR`,
`entry_to_fractal0_atr`, `stop_distance_atr`, `r_value_atr` считаются от
planned limit/stop/R полей; будущие PnL, exit и outcome поля запрещены.

Budget: `17` entry filters x `2` validation roles = `34` completed rows,
`permutation_repeats=200`. `val_select` выбирал filter family и cutoff; на
`val_eval` применялся только сохранённый `score_cutoff_on_val_select`, без
пересчёта topX по `val_eval`.

Аудит также выявил, что simple top50 baselines схлопывались в 0 сделок из-за
NaN cutoff. После исправления cutoff считается только по finite score rows:
`simple_stop_distance_top50` и `simple_r_value_top50` стали валидными и
конкурентными контролями.

Winner по `val_select`: `entry_quality_top10`,
`score_cutoff_on_val_select=0.36753163277225726`. На `val_select`:
`n_trades=196`, `PF=5.4967`, `BS_p05=3.9370`, `selected_fraction=0.0854`.
Permutation selection check PASS: `p=0.0597`. На `val_eval` тот же cutoff
оставил только `n_trades=53` и дал `PF=1.9543`, `BS_p05=0.9713`,
`mean_pnl_r=0.1671`, `max_drawdown_r=2.5825`, `selected_fraction=0.0231`,
`SL-rate=0.0943`.

No-mask baseline в том же run на `val_eval`: `n_trades=2298`,
`PF=2.5317`, `BS_p05=2.2865`, `mean_pnl_r=0.2638`,
`max_drawdown_r=9.1359`, `SL-rate=0.0627`.

Вывод: selected `entry_quality_top10` не пережил `val_eval` и не должен идти
в frozen rule. Diagnostic `val_eval` rows (`entry_avoid_sl_top50`,
`entry_quality_top50`, `simple_r_value_top50`) нельзя подставлять вместо
...
55. **Leaderboard closure частично закрыл внутренние блокеры, но не дал freeze-ready статуса.** Для тех же 11 fixed normalized leaderboard rows добавлен `ML/baseline/audit_leaderboard_closure.py`. Он не запускает новый search, сохраняет `original_rank`, пишет `ML/reports/leaderboard_closure_audit*` и оставляет maximum verdict `research_only`. Из saved `val_eval` trades computed: `time_calendar_status=COMPUTED` (`1587` rows) и `sequential_position_constraint_status=COMPUTED` (`33` policy rows). Не вычислимы из сохранённых artifacts: `stress_costs_status=NOT_COMPUTABLE_FROM_SAVED_ARTIFACTS`, `timezone_shift_status=NOT_COMPUTABLE_FROM_SAVED_ARTIFACTS`, `calendar_permutation_importance_status=NOT_COMPUTABLE_FROM_SAVED_ARTIFACTS`, `calendar_no_ml_baseline_status=NOT_COMPUTABLE_FROM_SAVED_ARTIFACTS`, `multi_seed_status=NOT_COMPUTABLE_FROM_SAVED_ARTIFACTS`. `provider_drift_status=NOT_IN_SCOPE`, `transfer_status=NOT_IN_SCOPE`, `locked_test=not_opened`. Решение: `LEADERBOARD_CLOSURE_INCOMPLETE_RESEARCH_ONLY`; следующий шаг только producer-level stress-cost resimulation, frozen timezone rescore, bounded multi-seed rerun для ровно 11 families или закрытие rich/fractal entry-quality как time-heavy research-only.

56. **Fixed11 producer-level internal closure computed all blockers and kept the branch risk-flagged.** `ML/baseline/fractal0_fixed11_internal_closure_rerun.py` пересчитал ровно 11 fixed normalized leaderboard rows с saved cutoffs и `--threads 24`: stress-cost `33` rows, timezone rescore `55`, calendar permutation `11`, no-ML calendar baseline `11`, multi-seed `55`, aggregate `11`. Итоговый JSON: `status=completed`, `overall_decision=FIXED11_INTERNAL_CLOSURE_RISK_FLAGS_RESEARCH_ONLY`, `locked_test=not_opened`. Timezone и multi-seed не дали risk flags, но stress-cost дал `12/33` risk rows, calendar permutation `4/11`, no-ML calendar baseline `11/11`; classification: `11/11 INTERNAL_CLOSURE_RISK_FLAGGED`. Практический вывод: rich/fractal entry-quality ветка остаётся time-heavy `research_only`; следующий шаг — закрыть её как time-heavy и написать узкий regime-filter reformulation plan, не открывая `locked_test`.

57. **Fixed11 candidate audit passed using forensic evidence from primary project artifacts.** Locked-test OOS evaluation opened `locked_test` once for 11 frozen normalized rules and reported all 11 passing PF/BS/sample-size gates (`PF=2.6747-3.3667`, verdict `candidate_check_required`). The independent audit now returns `candidate_audit_passed`: `0` errors, `13` warnings, `1` info. It accepts exact evidence from `docs/reports/2026-07-24-fractal0-fixed11-locked-test.md`, the locked-test protocol plan, `leaderboard_closure_audit_rules.csv`, locked-test JSON/CSV hashes and git history. Warnings remain because separate machine-readable freeze/policy JSON files were absent, source `split_roles` were sparse, `movement_plus_time` restoration was reconstructed from report text, low-N 2022 yearly slices are diagnostic edge-year disclosure, and `BS_p05` is still iid diagnostic. Result: proceed to mutual-correlation pruning for the 11 individual passed rules; MT4/tester parity and model card follow only for retained subset.

58. **Fixed11 mutual-correlation pruning reduced 11 rules to 5 retained working rules.** Pruning consumed only saved fixed11 locked-test CSV/JSON and candidate-audit JSON. It returned `overall_decision=pruning_passed`, `input_rule_count=11`, `retained_count=5`, `removed_count=6`, `pair_count=55`; pairwise verdict counts are `strong_duplicate=13`, `partial_overlap=42`, `unclear_or_complementary=0`. Representatives inside duplicate groups are selected by `BS_p05 / max_drawdown_r`, then robustness/trading tie-breakers. Retained rules: `rank02_time_only_linear_target_entry_ev_regression_top40`, `rank05_time_only_linear_target_entry_avoid_sl_top30`, `rank09_time_only_hist_gradient_boosting_target_entry_good_0_5r_top50`, `rank10_movement_plus_time_linear_target_entry_ev_regression_top50`, `rank11_movement_plus_time_linear_target_entry_good_0_5r_top50`.

59. **Fixed11 retained-subset MT4 parity перешёл из feasibility-блокера в per-rule diagnostic route.** Первичный feasibility-блокер показал, что одну общую строку `time;signal` нельзя честно использовать для всех 5 retained rules: она теряет `rule_id`, схлопывает одинаковые времена и не представляет противоположные сигналы на одном времени. Текущая рабочая политика поэтому не экспортирует все правила одной корзиной, а гоняет retained rules по одному `ML_RuleSlot`.

60. **Fixed11 retained-subset MT4 parity стал ближе после Python-style `MLClose`, но ещё не пройден.** `$o$imple.mq4`/`lib_ML_Signal.mqh` теперь поддерживают per-rule files: `ml_signals_fixed11_rule01..05.csv` и `ml_exits_fixed11_rule01..05.csv`. Fixed11 entry в MT4 ставит E3 limit order (`Open[0] ± ATR`), stop берёт из CSV как Python `S2` `protective_stop_price`, raw reversal отключён (`ML_AllowReversal=0`), а выход по `X2_ml_opposite_any_p0_50` представлен через exported `MLClose`. Последний ручной tester-run для `ML_RuleSlot=1` дал `ORDER_PLACED=1132`, `OPEN=1072`, `CLOSE=1072`, `MLClose=826`, `Timeout=223`, `StopLoss=23`, closed profit sum `62238.59`. Verdict остаётся `DIAGNOSTIC_ONLY / parity_in_progress`: у `798/1015` уникально сопоставленных сделок exit time ещё отличается от Python, обычно примерно на один H1-бар.

61. **Fixed11 fill-chronology audit shifted the blocker from MT4 timing to Python execution contract.** Analysis of slot 1 after stale handling showed that Python and MT4 do not share the same intra-H1 fill/exit chronology. Current Python runner records `execution_ohlc_usage=resolve_same_h1_bar_tp_sl_order_only`: M5 is used only to order SL/TP inside one H1 bar, not to timestamp limit fill and not to prove that same-H1 `MLClose` happens after fill. MT4 stale categories (`StalePendingAfterMLClose`, `StaleFillAfterMLClose`) explain why fresh tester PnL looked too good: many short negative Python trades are skipped or closed as stale in MT4. Old Python H1 also materially differed from current MT4 history from 2023 onward, so history drift and chronology mismatch were separated as two risks. Verdict remains `DIAGNOSTIC_ONLY`; Python locked-test artifacts must be rerun only after an explicit execution-contract fix.

62. **Fixed11 current-OHLC rerun separated data drift from same-H1 chronology risk.** Rerun used current `DATA/XAUUSD_H1_OHLC.csv` and `MT/MQL4/Files/XAUUSD_M5_OHLC.csv`, kept `DATA/Nero_XAUUSD_test_labeled.csv` unchanged, did not change Python execution logic, and wrote separate artifacts under `ML/reports/fractal0_fixed11_rich_entry_locked_test_current_history*`. Aggregate fixed11 changed materially: trades `14507 -> 13039`, PnL R `4429.782419 -> 4065.034595`, PF `3.097520 -> 3.116313`, `hold_bars=0` `5100 -> 4495`. Retained slot 1 changed from `1196` to `1091` trades and PnL R `395.026902 -> 339.192111`, but current-history still has `368` same-H1 fill/exit trades and `368` `hold_bars=0` trades with `hold0` PnL `-98.196808R`. Conclusion: fresh OHLC matters, but it does not solve the core blocker. Next step is a separate chronology-fix plan; do not export current-history artifacts as MT4 parity evidence.

63. **Fixed11 Python H1 chronology fix invalidated the old positive fixed11 chain.** The corrected runner stores M5 `fill_execution_time`, excludes `bars_since_fill=0` from working ML-exit train/score rows, keeps future exit fields target/diagnostic-only, resolves same-H1 SL/TP only after factual M5 fill, and executes `ML_CLOSE` at the next H1 open after the decision bar. Diagnostic artifacts under `ML/reports/fractal0_fixed11_rich_entry_locked_test_h1_chronology_fix*` show `same_h1_ml_close` `4070 -> 0`, `hold_bars=0` `4495 -> 488`, all `14387` new trades with `fill_execution_time_source=m5_touch`, but the old edge disappears: PnL R `4065.034595 -> -530.513260`, PF range `2.820656-3.424707 -> 0.819373-0.938880`, `kept_candidates=0`. Verdict is `DIAGNOSTIC_ONLY`: M5 was not used as ML input, MT4 parity is not proven, and the next step is either fixed11 post-mortem or diagnostic-only MT4 slot parity.
64. **MT5 execution-loop diagnostic prototype prepared.** Existing `MT/MQL5/Experts/$o$imple.mq5` remains the primary target and compiles with MetaEditor `0 errors, 0 warnings`. Added default-off MT5 `Nero.csv` producer (`InpMT5_ExportNero=false`) with 23-field fractal payload including `Shift`, entry-only Python export, MT5 signal/event schemas, diagnostic event logging, post-fill feature contract, parser metrics and manual tester runbook. Verdict remains `DIAGNOSTIC_ONLY`: MT5 `Nero.csv` parity is `UNKNOWN`, Strategy Tester runtime was not run, and tester execution does not prove feature-leakage safety.
...
- [2026-07-23-fractal0-leaderboard-cost-calendar-sequential-multiseed-closure.md](../../docs/reports/2026-07-23-fractal0-leaderboard-cost-calendar-sequential-multiseed-closure.md) — closure для 11 fixed leaderboard rows: calendar и sequential-position computed, stress-cost/timezone/multi-seed not computable from saved artifacts, `locked_test` not opened
- [2026-07-23-fractal0-fixed11-internal-closure-rerun.md](../../docs/reports/2026-07-23-fractal0-fixed11-internal-closure-rerun.md) — producer-level fixed11 closure rerun: all diagnostics computed, `11/11` rows risk-flagged by stress/calendar evidence, `locked_test` not opened
- [2026-07-24-fractal0-fixed11-locked-test.md](../../docs/reports/2026-07-24-fractal0-fixed11-locked-test.md) — locked_test OOS evaluation of 11 frozen normalized rich-entry rules with M5 execution contract: all 11 pass PF/BS/sample-size gates, PF range `2.6747-3.3667`, verdict `candidate_check_required`
- [2026-07-25-fractal0-fixed11-candidate-audit.md](../../docs/reports/2026-07-25-fractal0-fixed11-candidate-audit.md) — independent read-only audit of fixed11 locked-test artifacts: `candidate_audit_passed`, 0 errors, 13 warnings and 1 info; next step is mutual-correlation pruning
- [2026-07-27-fractal0-fixed11-mutual-correlation-pruning.md](../../docs/reports/2026-07-27-fractal0-fixed11-mutual-correlation-pruning.md) — mutual-correlation pruning of 11 passed fixed rules: `pruning_passed`, retained subset 5 rules selected by `BS_p05 / max_drawdown_r`, 6 direct strong duplicate drops; next step is MT4/tester parity for retained subset
- [2026-07-27-fractal0-fixed11-retained-subset-mt4-parity.md](../../docs/reports/2026-07-27-fractal0-fixed11-retained-subset-mt4-parity.md) — retained-subset MT4 parity diagnostic: `DIAGNOSTIC_ONLY / parity_in_progress`; per-rule signal/exit CSV route implemented, slot 1 is much closer after `MLClose`, but parity is not passed because MT4 exits usually lag Python by about one H1 bar
- [2026-07-29-fixed11-python-mt4-fill-chronology.md](../../docs/reports/2026-07-29-fixed11-python-mt4-fill-chronology.md) — fill chronology audit: Python M5 execution OHLC only resolves same-H1 SL/TP ordering, not limit fill timestamp or same-H1 `MLClose` ordering; old H1 history drift and execution-contract mismatch both keep the result `DIAGNOSTIC_ONLY`
- [2026-07-29-fixed11-current-history-rerun.md](../../docs/reports/2026-07-29-fixed11-current-history-rerun.md) — current H1/M5 OHLC rerun of fixed11 locked-test without logic changes: aggregate metrics changed materially, but slot 1 still has `368` same-H1/`hold_bars=0` trades, so the next step is a separate Python chronology-fix plan
- [2026-07-29-fixed11-python-h1-chronology-fix.md](../../docs/reports/2026-07-29-fixed11-python-h1-chronology-fix.md) — corrected Python H1 chronology contract: M5 limit fill timestamp, no working `bars_since_fill=0` ML-exit rows, no same-H1 `ML_CLOSE`; diagnostic rerun has `PF max=0.938880`, `kept_candidates=0`
- [2026-07-29-mt5-execution-loop-migration.md](../../docs/reports/2026-07-29-mt5-execution-loop-migration.md) — MT5 execution-loop diagnostic prototype: `$o$imple.mq5` compiles, entry-only signal export/event parser added, MT5 `Nero.csv` parity and tester results still pending
