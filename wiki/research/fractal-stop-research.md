...
- идея времени до пробоя не закрыта, но старый oracle gate слаб и требовал исправления;
- root cause идентичных метрик найден: `reg:pseudohubererror` выдавал константу вне диапазона, clipping превращал все предсказания в `1.0`;
- код исправлен на `reg:squarederror`, в метрики добавлен `pred_summary`;
- старый JSON superseded; нужен полный rerun Stage 5.2;
- `time_only` не является winner, он выбран только из-за tie-break среди одинаковых нулевых метрик;
- следующий шаг — полный rerun; если результат снова слабый, тогда проверять постановку с учётом цензуры (`breach_after_k`, ordinal buckets или survival-style), а не новый широкий перебор по `H6_off05`.
...
34. **Stage 5.2: root cause старой аномалии найден.** Старый JSON дал идентичные метрики на 7 профилях: Spearman `0.0000`, AUC `0.5000`, MAE хуже constant baseline. Debug показал причину: `reg:pseudohubererror` на полном `time_only` split выдавал почти константный raw-прогноз `-171.076`, clipping превращал все предсказания в `1.0`. `reg:squarederror` на тех же признаках даёт variance и ненулевое ранжирование. Старый JSON superseded; нужен полный rerun Stage 5.2.
...
- ~~Почему Stage 5.2 дал одинаковые нулевые метрики для всех профилей?~~ Root cause найден: `reg:pseudohubererror` + clipping. Следующий вопрос — как выглядит полный rerun на `reg:squarederror`.
...
- [2026-06-25-stage5_2-time-to-breach-regression.md](../../docs/reports/2026-06-25-stage5_2-time-to-breach-regression.md) — Stage 5.2: регрессия времени до пробоя; старый JSON superseded из-за `reg:pseudohubererror` + clipping, код исправлен на `reg:squarederror`, нужен полный rerun
