...
> Фрактальные признаки предсказывают пробой уровня, oracle (проверка потолка) показывает высокий диагностический потолок механики, но RF/XGBoost/Transformer пока не дают устойчивого торгового или модельного превосходства. Stage 5.0d закрыл постановку `H6_off05` на текущих профилях. Stage 5.0e показал: меньший Transformer действительно уменьшает признаки переобучения, но не догоняет XGBoost на тех же признаках. Stage 5.0f добавил: явный распад сигнала во времени тоже не доказан, но и устойчивость не подтверждена. Итог по ветке `H6_off05` остаётся диагностическим и неопределённым.
...
Stage 5.0f не открывал нового кандидата и не переоткрывал `H6_off05`. Он проверял, достаточно ли данных, чтобы честно сказать: сигнал распадается во времени, или наоборот остаётся устойчивым.
...
- Для обеих целей `rolling` не дал ни одного решающего года, где его нижняя граница качества была бы выше верхней границы `fixed`.
- `time_only` не оказался не хуже фрактальных наборов ни по одной цели.
- `structure_only` остался близок к `base_raw_plus_time` по обеим целям.
- По buy виден более сильный нисходящий рисунок в `anchored`, но он всё равно не даёт полного и жёсткого вывода о распаде сигнала.

Итог Stage 5.0f:

- не удалось доказать, что сигнал явно распадается во времени;
- не удалось и подтвердить, что он устойчив;
- значит, состояние ветки `H6_off05` после 5.0f остаётся диагностическим и неопределённым.
...
28. **Природа отрицательного результата не установлена.** Вердикт `h6_off05_target_exhausted` не различает: (H1) сигнал слабый/отсутствует, или (H2) сигнал затухает во времени и сравнение на одном val-окне локально. Stage 5.0f добавил важное уточнение: жёсткого вывода не получилось ни в пользу H1, ни в пользу H2.
...
31. **Stage 5.0f не подтвердил ни распад сигнала во времени, ни его устойчивость.** `rolling` не показал жёсткого преимущества над `fixed`, `time_only` не оказался не хуже фрактальных наборов, а `structure_only` остался близок к `base_raw_plus_time`. Значит, остаточный сигнал нельзя честно свести только к календарю, но и реабилитировать ветку нельзя.
...
- **Нужен ли отдельный подтверждающий цикл на новом периоде `2026+`?** Stage 5.0f не дал жёсткого ответа. Если проекту нужен окончательный вывод по ветке, потребуется новый период и короткий заранее замороженный протокол без повторного использования `2023-2025`.
