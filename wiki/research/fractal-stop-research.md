...
- **Что делать после movement-filter stage?** Уже ясно, что `next open after signal_time` не работает как direction target ни при старом target от `fractal0_price`, ни при новом target от фактического `entry_open`, ни после closeout shortlist, ни после более мощных табличных моделей, ни после bounded sequence-проверки. Amplitude / movement-regime цель от фактического исполнения тоже не дала сложного winner-а: её объясняют главным образом `time+ATR`, а не фрактальная структура. Новый простой movement-filter уже найден и честно ограничен одним research-only winner-ом. Открытым остаётся не новый wide search, а только узкая репликация/заморозка `simple_combined / extra_trees_small / H3 / top_fraction=0.05` или отдельная entry-механика, привязанная к `fractal0_price` (`retest-entry`, `limit-entry`, касание зоны).
