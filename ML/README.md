...
| [entry_path_direct_direction_targets.py](entry_path_direct_direction_targets.py) | Target families A/C/D для direct-direction; A/C требуют raw up/dn/ATR, D строится из OHLC | source/OHLC CSV → target masks | ✅ |
| [fractal_level_feature_builder.py](fractal_level_feature_builder.py) | Fractal-level nearest-k/zones признаки с raw price / raw ATR distance contract | labeled frame + optional raw frame → feature frame | ✅ |
| [benchmark_entry_path_binary_direction.py](benchmark_entry_path_binary_direction.py) | Validation-first binary BUY/SELL direct-direction benchmark с protocol gates и selection_decision | train/validation + raw source + OHLC → reports/direct_direction_* | ✅ |
...
# Обучение по умолчанию идёт на CPU; причина:
