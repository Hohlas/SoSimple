...
Основание: независимый audit 2026-07-25 завершён с
`candidate_audit_blocked`. Сам `locked_test` bundle по метрикам не развалился,
но статус нельзя повышать из-за отсутствия pre-open freeze artifacts,
неполного split disclosure, low-N yearly edge slices без
`DIAGNOSTIC_ONLY`-классификации и неструктурированного
`movement_score_restoration` disclosure.

Цель: снять только audit blockers без нового выбора по `locked_test` и без
изменения frozen candidate rules.

Следующий шаг после аудита:

- восстановить или честно зафиксировать отсутствие pre-open freeze/policy;
- добавить structured split disclosure (`val_select`, `val_eval`, boundaries);
- добавить structured movement restoration disclosure для 4
  `movement_plus_time` rules;
- пометить edge-year low-N slices как `DIAGNOSTIC_ONLY`;
- только потом повторить candidate audit.
...
Статус: `PARKED_BY_CANDIDATE_AUDIT_BLOCKERS`.
...
Статус: `PARKED_AFTER_CANDIDATE_AUDIT_PASS`.
