...
57. **Fixed11 locked test opened the frozen bundle once and moved status only to `candidate_check_required`.** Отдельный runner `run_fractal0_fixed11_rich_entry_locked_test.py` открыл `locked_test` для ровно 11 fixed normalized rich-entry rules с M5 execution contract. Все 11 rules прошли базовые individual gates: PF range `2.6747-3.3667`, `BS_p05` range `1.9273-2.9239`, `kept_candidates=11`, BUY PF range `3.6196-5.1218`, SELL PF range `1.9485-3.0798`. Это не был новый search и не был portfolio selection; вердикт этапа остался только `candidate_check_required`.
58. **Independent candidate audit blocked status promotion despite strong locked-test metrics.** Новый read-only audit `audit_fractal0_fixed11_candidate.py` подтвердил source hashes и 11-rule consistency, но дал `overall_decision = candidate_audit_blocked`. Причины: отсутствуют `fractal0_fixed11_locked_test_freeze.json` и selection-policy artifact, в source JSON нет `val_select`/`val_eval` split disclosure и `split_boundaries`, не раскрыт `correlation_pruning_status`, 6 yearly edge slices (`2022`) имеют `n_trades < 30` без `DIAGNOSTIC_ONLY`, а 4 `movement_plus_time` rules не имеют structured `movement_score_restoration` contract. Практический вывод: correlation pruning, MT4/tester parity и model card запрещены до устранения этих disclosure blockers без изменения frozen rules.
...
- [2026-07-25-fractal0-fixed11-candidate-audit.md](../../docs/reports/2026-07-25-fractal0-fixed11-candidate-audit.md) — independent read-only audit of the locked-test bundle: hashes and 11-rule bundle are consistent, but status promotion is blocked by missing pre-open freeze artifacts, missing split disclosure, low-N yearly edge slices and missing structured movement restoration disclosure
