...
- **topics**: `fractal0`, `fixed11`, `candidate_audit`, `freeze_contract`, `split_disclosure`
- **summary**: Added a read-only audit CLI for `fractal0_fixed11_rich_entry_locked_test*` and ran it on the current bundle. The audit confirmed hashes and 11-rule consistency, but blocked status promotion because pre-open freeze evidence and structured split/movement disclosures are incomplete.
...
- **decision**: `candidate_audit_blocked`; correlation pruning, MT4/tester parity and model card are forbidden until the audit blockers are resolved without changing frozen rules.
- **notes**: blockers = missing `fractal0_fixed11_locked_test_freeze.json` / selection policy, missing `val_select`+`val_eval` split disclosure, 6 low-N yearly edge slices without diagnostic classification, missing structured movement-score restoration for 4 `movement_plus_time` rules.
