...
| Функция | Описание |
|---------|----------|
| `parse_fractal(str)` | Смысловой парсинг 23-полевой raw-строки фрактала → dict; integer-like поля допускают только целую запись (`1`, `1.0`), дробные нормализованные значения отвергаются |
...
- `parse_fractal()` в `label_signals.py` — смысловой parser для разметки. Он намеренно отвергает дробные значения в `strong/break/count`, чтобы не применять разметочный parser к нормализованным CSV. Для ML-признаков такие каналы нужно читать как float-признаки отдельным feature extractor.
