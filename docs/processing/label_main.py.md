...
5. **Нормализация (Rowwise)**: `normalize.py` — признаки каждой строки + up/dn таргеты. Сохраняет per-row `[brk, cap]` (p85/p99 из пула 606 значений). Для live-safe retrain использовать `--exclude-predict-from-front-back-pool`, чтобы `predict`, рассчитанный из будущего, не менял масштаб `front/back`.
