...
Проверка масштаба финальных входов выполняется до обучения. Если она дала `ERROR` или `WARNING`, нельзя молча продолжать и интерпретировать метрики как качество модели.

1. `PADDING_NOT_ZERO` — блокер. Исправить mask/padding: padding не участвует в fit scaler и после normalization остаётся `0.0`. Перенормализация не считается исправлением.
2. `NaN` / `Inf` — блокер. Найти источник: парсинг, деление на ноль, некорректный `ATR`, недопустимый логарифм или missing value. Исправить preprocessing/feature builder и пересобрать входы.
3. `TAIL_GT10` / `TAIL_GT20` — предупреждение, не автоматический баг. Указать, какой признак дал хвост, и до просмотра holdout-метрик зафиксировать одну из диагностических обработок:
   - оставить как есть с обоснованием;
