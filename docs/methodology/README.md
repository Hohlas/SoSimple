...
## Как использовать
Методика задаёт процесс, а не рецепт. У агента есть свобода в выборе архитектуры и способов реализации.
Методика применяется к любому новому ML-кандидату: классификации направления, регрессии MFE/MAE, take/skip, triple barrier, фильтру сигналов, execution-policy или модели выбора стороны сделки. Для торговых постановок с явным правилом входа/выхода до обучения модели выполняется oracle-preflight: диагностическая проверка теоретического потолка механики.
...
| Предварительно проверяешь, имеет ли торговая механика смысл при идеальном знании будущих labels и выбираешь перспективную side/H/off-зону | [06b-oracle-preflight.md](06b-oracle-preflight.md) |
