...
## Примечание: календарные признаки

Календарные признаки (hour, day-of-week sin/cos) технически live-safe: они известны на момент `decision_time` без заглядывания в будущее. Однако они могут кодировать паттерны, специфичные для конкретного провайдера данных или часового пояса, и не переноситься на другие периоды или источники.

Если permutation importance календарных признаков >30% общей важности модели — это не доказывает утечку, но является red flag: модель может быть календарным фильтром, а не фрактальным классификатором. В этом случае требуется:
- Проверить устойчивость PF при сдвиге часового пояса на ±1 час.
- Проверить на данных другого провайдера или инструмента.
- Документировать риск в model card.
