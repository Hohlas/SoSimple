...
1. Зафиксировать `decision_time`: open/close бара, таймфрейм, инструмент, момент доступности feature snapshot, момент записи строки, задержку watcher/runner и самый ранний исполнимый момент входа.
...
| 19 | Entry price исполним после доступности признаков | Backtest должен входить не раньше, чем live реально может отправить ордер | В отчёте указаны feature-ready time, row-write time, watcher polling/processing delay и executable entry convention | Label/evaluation использует `Close[row]` или `Open[row+1]`, хотя строка/сигнал появляется только после этого момента |
