...
Если permutation importance календарных признаков >30% общей важности модели — модель, вероятно, является календарным фильтром, а не фрактальным классификатором. В этом случае требуется:
