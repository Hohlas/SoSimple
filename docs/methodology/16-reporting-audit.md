...
- В отчёте явно указан уровень этапа: поисковый или проверочный. Для поискового — явно отмечено, что результат не может быть кандидатом без нового проверочного цикла. Для проверочного — явно отмечено, является ли прогон повторной проверкой гипотезы, порождённой предыдущим этапом, или независимым открытием.
