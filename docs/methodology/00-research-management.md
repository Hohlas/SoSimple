...
5. Заранее задать gate-критерии: PF, число сделок, минимальное число сделок в год, просадка, отрицательные годы, BUY/SELL устойчивость, baseline uplift. Sequential PF (SeqPF) не использовать как gate-критерий — эта метрика невалидна (см. Типовые причины ложных выводов).
