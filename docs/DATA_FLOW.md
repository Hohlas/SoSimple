...
`Up/Dn` формируются в `lib_PIC.mqh`, а не создаются Python-постобработкой.

Механика:

- при создании нового уровня `NEW_LEVEL()` инициализирует `F[f].Up[5]` и `F[f].Dn[5]` нулями;
- дальше `LEVELS_FIND_AROUND()` на каждом уже прошедшем баре обновляет максимум движения цены вверх/вниз от цены фрактала;
- `NERO_CSV_CREATE()` записывает в текущую строку только значения, накопленные к моменту этой строки.

Поэтому `Up/Dn` в строке `Nero.csv` — это не знание будущего относительно этой строки. Это историческая реакция цены на уже существующий уровень, похожая по смыслу на `Back`, но ограниченная фиксированными горизонтами 3/6/12/24/48 баров.

Практические следствия:

- использовать `Up/Dn` как исторические входные признаки допустимо, если модель получает только текущую строку и прошлые фракталы;
- использовать `Up/Dn` как целевые значения тоже допустимо, когда они извлекаются из строки для supervised-разметки;
- ошибка возникает только если искусственно подмешать в строку значения `Up/Dn`, рассчитанные по барам после момента этой строки.
...
- **UPDN_TARGET_COLUMNS** (нормализуются теми же параметрами, но по умолчанию не входят в пул): `up_3`, `dn_3`, `up_6`, `dn_6`, `up_12`, `dn_12`, `up_24`, `dn_24`, `up_48`, `dn_48`
- **Логика по умолчанию**: p85/p99 считаются только по фрактальным `UPDN_FIELDS` текущей строки; top-level targets затем нормализуются этими параметрами, но не могут менять scaling model-input фракталов. Legacy-режим `include_targets_in_updn_pool=True` допустим только для воспроизведения старых артефактов.
...
- **Top-level targets не включаются в пул p85/p99 по умолчанию** — иначе target-only значения могут менять нормализованные фрактальные Up/Dn model inputs.
