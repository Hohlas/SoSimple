...
В тестере `SERVICE.mqh::INPUT_FILE_READ()` читает `#.csv`. Если `BackTest > 0`, используется только строка, у которой `EXP[e].Back == BackTest`.

Практический нюанс: текущая реализация сначала читает `#.csv` через `FileReadString()`, и если первая ячейка не похожа на рабочую строку, она делает `continue` без дочитывания остатка строки. Поэтому заголовок фактически перебирается по ячейкам, а `BackTest` для первой рабочей строки равен:

```text
количество полей заголовка + 2
```

В текущем telemetry-файле 96 полей заголовка, поэтому первая рабочая строка выбирается через `BackTest=98`.
