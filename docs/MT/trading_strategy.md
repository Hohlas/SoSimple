...
На 2026-04-28 текущая рабочая строка временно переключена на `XAUUSD1` для
ускоренного прогона стартовой и online-логики на минутках. Это не новый
торговый режим, а временный диагностический preset.
...
6. Если файл изменился, `ML_TRADE()` перезагружает сигналы и начинает работать с
   новым рядом.
7. Сверка запускается отдельным Python CLI: `ML.telemetry_daily_reconciliation`
...
Важное ограничение, найденное 2026-04-28:

- эксперт уже пересобирает и дописывает `Nero.csv` по истории и новым уровням;
- watcher корректно строит prediction CSV из raw `Nero.csv`;
- но текущий online `Nero.csv` пишет `signal=0` и `predict=0` во всех строках;
- diagnostic exporter `telemetry_frequency_v1` берёт направление сделки именно
  из знака `predict`, поэтому при нулевом `predict` итоговый `ml_signals.csv`
  будет содержать только `signal=0`.

Следующий архитектурный вопрос: восстановить online-формирование направления
сигнала так, чтобы оно соответствовало offline/test pipeline. Вероятное место
расхождения - постобработка `Nero.csv`, которая использовалась перед обучением
и тестированием, но пока не воспроизведена в live-export контуре.
