...
`OnInit() SoSimple.V260.334`. Версии ниже `260.333` могут читать `PARAMS` как
...
перед первой записью нового tester-прогона.

### Онлайн-торговля

Для online/forward запуска ставить:

```text
BackTest=0
```

Причина: в online-режиме советник должен прочитать все рабочие строки
`#.csv` и сам выбрать те, которые подходят под текущий график по символу,
периоду, риску и `Magic`. `BackTest=2` нужен только тестеру, где мы
принудительно выбираем одну строку файла.
...
Для online-режима не нужен отдельный `ml_signals.ready`, если Python публикует
сигналы атомарно:

1. Python строит полный runtime export в `ML/reports/.../runtime_ml_signals.csv`.
2. Для MT4-файлов он сохраняет уже записанные строки `ml_signals.csv` и
   добавляет только строки новее текущего хвоста.
3. Обновлённый файл сначала пишется во временный `ml_signals.csv.tmp`.
4. После полной записи временный файл заменяет `ml_signals.csv`.
5. MT4 на новом баре проверяет время изменения `ml_signals.csv`.
6. Если файл изменился, `ML_TRADE()` перезагружает сигналы и пишет в лог
...
обязателен. Старые online-строки при этом не меняются задним числом, поэтому
файл можно использовать как журнал фактических решений Python.
...
3. Когда появляется новый завершённый бар, Python читает новые строки,
   пересчитывает признаки, применяет модель и формирует полный runtime export.
4. Python публикует в MT4 append-only `ml_signals.csv`: старые строки остаются,
   добавляются только строки новее хвоста файла.
...
```text
MT/MQL4/Files/ML_Trade_Events_<NAME>_<magic>.csv
```
