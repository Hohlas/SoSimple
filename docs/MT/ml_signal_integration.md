...
- для fixed11 retained subset может выбирать один из пяти файлов через
  `ML_RuleSlot`.
...
Для fixed11 retained subset можно запускать правила по одному. В этом режиме
`ML_RuleSlot` выбирает файл:

| `ML_RuleSlot` | Файл |
|---:|---|
| `0` | `ml_signals.csv` |
| `1` | `ml_signals_fixed11_rule01.csv` |
| `2` | `ml_signals_fixed11_rule02.csv` |
| `3` | `ml_signals_fixed11_rule03.csv` |
| `4` | `ml_signals_fixed11_rule04.csv` |
| `5` | `ml_signals_fixed11_rule05.csv` |

В tester это переключается через `BackTest=2..6`: `BackTest=2` выбирает
первую рабочую строку после заголовка `#.csv`, и дальше каждая строка задаёт
соответствующий `ML_RuleSlot`.

Важно: это разделяет пять правил по отдельным прогонам, но не решает
автоматически дубли внутри одного правила. Если в одном правиле несколько
сделок на один `signal_time`, export должен явно описать, как они превращаются
в один `time;signal`.

Для текущего ручного tester patch файлы `ml_signals_fixed11_rule01.csv` ...
...
- при `--copy-to-mt4` атомарно копирует один и тот же результат в
  ...
  `ml_signals.csv`;
- при `--copy-to-mt4 --append-to-mt4` сохраняет старые строки MT4-файлов и
  добавляет только строки новее текущего хвоста; этот режим используется
  online watcher-ом;
...
`MLP NO_SIGNAL` - это отладочная запись для бара без строки сигнала, а не
ошибка. По умолчанию она выключена через `ML_LogNoSignal=false`; включать её
стоит только при ручной проверке конкретного пропущенного времени.
