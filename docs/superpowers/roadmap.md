...
### 1. Regression Up/Dn target foundation

**Задача:** проверить новую цель на будущий ход вверх/вниз без привязки к
фрактальному стопу.

**Контекст:** Stage 6.2 и `range_w1_atr` post-mortem закрыты как
`DIAGNOSTIC_ONLY`. Проверенная H12 OHLC-window family не дала устойчивого
торгового преимущества, поэтому следующий шаг — новая постановка цели, а не ещё
одна мелкая вариация тех же OHLC windows.

**Ограничение:** не смешивать новую цель с новым широким перебором признаков.
Сначала нужен фундамент target/label contract, baseline и честный split.
