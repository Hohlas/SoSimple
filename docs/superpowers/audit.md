...
> **Статус после исполнения (2026-08-07):** пункты 1–10 ниже закрыты исполнением
> плана `2026-08-03-mt5-multi-position-closeout.md` (Tasks 4–9 выполнены:
> multi-ticket tracker в `lib_ML_Signal.mqh`, ulong-перегрузка `OrderSelect` в
> `MQL4Compat.mqh`, `--force-rerun`/`--smoke-only` в `run_mt5_batch.py`,
> compile gate `0 errors, 0 warnings`, smoke max=1/2/16 с `UNEXPLAINED=0`).
> Раздел «Статус выполнения плана (факт)» ниже отражает состояние на момент
> аудита (Tasks 4–9 не выполнены) и сохранён как исторический срез.
> Итоговый отчёт: `docs/reports/2026-08-03-mt5-multi-position-closeout.md`.
