...
Идея в том, что мы ставим стоп лосс за пик фрактала0, в этом случае пробитость пика фрактала0 Brk_h означает, после входа цена коснулась уровня, за которым стоит наш SL и это эквивалентно срабатыванию стопа.                         
Если модель предсказывает, что за 6 бар уровень (за которым у нас стоп) не будет пробит, и за эти же 6 баров цена сходит на pred_fav_6, то мы заходим в сдлелку. Дополнительно проверяем, удовлетворяет ли нас размер pred_fav_6, например pred_fav_6>2, или лучше pred_fav_6>SL_val. Тейк профит ставим чуть ниже pred_fav_6*ATR, стоп ставим за пик фрактала0 и входим в сделку на открытии следующего бара Open[row+1].
 SL_val=Open[row+1]-fractal0.price-ATR*0.5 (для buy), тут ATR*0.5 - страховка, чтобы стоп не выбило шумом. 
