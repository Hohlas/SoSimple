...
**Architecture:** Не менять `lib_PIC.mqh` и формат `Nero.csv`. Новый слой строится на уже экспортируемых 22-полевых фракталах и добавляет производные признаки по свежим окнам 5/10/20/50/100. Поля `Up/Dn` не используются, чтобы не смешивать входные данные с будущим результатом.
...
- Не менять `Nero.csv`.
- Не использовать `Up/Dn` поля как входные признаки.
