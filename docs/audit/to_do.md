...
- [x] Для каждого подготовленного CSV сохранять рядом файл описания схемы, например `Nero_train_labeled.schema.json`: версия фрактала, число полей, порядок полей, версия тензора, число признаков на фрактал, источник/создатель данных, режим нормализации.
- [x] Парсеры и обучение должны проверять схему до работы с данными. Если код ожидает одну версию, а CSV имеет другую, выполнение должно останавливаться с понятной ошибкой. Молчаливое запасное значение допустимо только для явно разрешённого режима старого формата.
- [x] Название файла может дублировать версию (`*.fractal_v23.csv`), но не должно быть единственным источником истины: порядок полей и число признаков должны проверяться программно.
...
- [x] `C_path_long_nf0` (без fractal0): PF=1.488 — заменён на `only_path` (каналы 10–19) в fractal_ablation.py (PF=3.97 test).
- [x] Оба path_long-варианта дают 0 сделок в 2019 — проверено на новой абляции: все годы имеют сделки.
- [x] Проверка разброса path_long — заменена годовой разбивкой в fractal_ablation.py.
...
- [x] GridSearch RF: validation PF=12.96 (n=200, d=20, leaf=1) vs baseline 11.38 (n=100, d=10, leaf=1), Δ=+1.58. Глубина — главный фактор. Результат диагностический (10 фракталов, 10K строк), не production-дефолт. Скрипт: `ML/baseline/rf_gridsearch.py`.
- [x] OOB-сходимость: R² от 0.29 (n=10) до 0.44 (n=300), основной прирост в первых 50 деревьях. 100 деревьев уже близко к насыщению.
- [x] max_depth=None с leaf=5: PF=12.02 на полном train (Δ +0.64 к baseline). Хуже, чем d=20 с leaf=1 — глубина ограничивает сильнее, чем листья.
...
- [x] К направлениям добавить up_3..dn_48 (как flat-признаки, не агрегаты). Закрыто как недопустимое: плоские up/dn в размеченном CSV — целевые переменные, вычисленные по будущему движению цены (label_updn отслеживает fractal0 вперёд). Использовать их как признаки = модель видит ответ. Альтернатива проверена: fractal-level path-каналы 10–19 в тензоре (`only_path` в абляции).
