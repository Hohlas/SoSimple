...
### Oracle-диагностика (проверка потолка): отделение модели от механики
...
| sell_H12_off02 | 22.01 | 23.55 | ∞ |
| buy_H12_off05 | 23.52 | 10.40 | ∞ |
| sell_H12_off05 | 13.94 | 12.16 | ∞ |

**Все oracle-режимы и все комбинации на val: 0 убыточных лет при canonical spread 0.20.** Один срез (`sell_H12_off02|perfect_fav`) имеет только 21.8 сделок/год, поэтому его нельзя использовать для выбора постановки без дополнительного подтверждения.

Ключевые наблюдения:
- `perfect_both` → PF=∞ везде: **у механики высокий диагностический потолок**. При идеальном знании breach и fav_val на val не осталось убыточных сделок.
- `perfect_breach` → PF=8–28: идеальное знание пробоя стопа даёт фактор PF 10–30× относительно RF baseline (0.6–0.9).
- `perfect_fav` → PF=7–24: идеальное знание благоприятного хода также даёт сильный прирост, но в одном H12 SELL-срезе число сделок ниже минимального порога 30/год.
- `perfect_breach` сохраняет высокий PF при большем числе сделок во всех H/off/side-срезах, поэтому классификация пробоя остаётся главным практическим узким местом.

**Вывод oracle: проблема текущего Stage 2 результата в модели (RF), а не в отсутствии диагностического потолка у механики.** Oracle использует будущие labels и не доказывает торговую пригодность стратегии, но показывает, что связку «стоп за уровнем + TP от fav» имеет смысл исследовать дальше с более сильными признаками или моделью.
...
2. **Oracle показывает высокий диагностический потолок механики**: при идеальном знании breach PF=8–28, при идеальном fav PF=7–24, при обоих — PF=∞ (0 убыточных сделок на val). Это не торговое доказательство, а указание, что текущий FAIL связан с извлечением сигнала моделью.

3. **RF не извлекает доступный сигнал**: gap между oracle PF и RF PF (0.6–1.0) составляет фактор 10–30×. Нужны более сильные признаки или более мощная модель.

4. **Breach-знание практичнее как следующий фокус**: `perfect_breach` даёт PF 8–28 и во всех срезах держит ≥30 сделок/год. `perfect_fav` тоже силён, но один H12 SELL-срез ниже порога сделок. Приоритет — улучшить классификацию пробоя, не закрывая fav-регрессию.
...
Oracle-тест показал, что у торговой механики есть высокий диагностический потолок (PF=∞ при идеальном знании на val), а RF не может извлечь этот сигнал. Причины:

1. **Breach-сигнал слишком слаб для фильтрации**: AUC 0.65 означает, что даже лучший порог даёт много ложных пропусков. При breach rate 40–60%, значительная доля «не-пробойных» сделок всё равно проходит фильтр (RF даёт breach_prob < p для многих реальных пробоев), а значительная доля «пробойных» отсеивается. Gap между `perfect_breach` (PF=8–28) и RF (PF=0.6–1.0) показывает, что breach-классификатор — основное узкое место.
...
3. **Стоп за fractal0 — широкий**: stop_val 1.0–2.5 ATR. При TP = pred_fav × tp_fraction, RR часто < 1.0. Но oracle показывает, что даже с такими стопами у механики есть диагностический потолок — нужен более точный breach-фильтр и/или более точный fav-прогноз.
...
Oracle-диагностика показала: **проблема в текущей RF-модели, а не в полном отсутствии потенциала механики**. Рекомендуется **Stage 3: улучшение breach-классификатора**:
