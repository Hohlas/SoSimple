...
1. **Разные способы кодирования фракталов не превосходят стандартное base-кодирование.** Базовый профиль `base_raw_plus_time` сам содержит 9 структурных фрактальных признаков (`direction`…`impulse`) на 100 фракталах — это и есть фрактальный сигнал. Проверенные 9 профилей варьируют нормализацию (relative price, absolute price/ATR, price_unit), выборку (all100, nearest40, corridor) и tokenization, но ни один не превосходит base на ≥0.02 AUC. На buy-цели ни один профиль не превосходит base вовсе (дельты ≤ −0.0006). Вывод относится к способам кодирования, не к наличию фрактального сигнала как такового.
...
7. **Сигнал базовой модели деградирует на holdout по годам.** Yearly breakdown `base_raw_plus_time` (из JSON): sell 2023=0.676 → 2024=0.679 → 2025=0.631 → 2026=0.556 (n=316, почти случайно); buy 2023=0.695 → 2024=0.656 → 2025=0.609 → 2026=0.653 (n=293). Это согласуется с Stage 5.0 (0.646→0.514) и Stage 4.7 walk-forward (PF<1.0 на 2023-2026 во всех режимах: expanding 0.84, anchored 0.897, rolling 0.942, warmstart 0.882). Вывод `h6_off05_target_exhausted` основан на сравнении профилей на val 2021-2022; темпоральная деградация базового сигнала не проверялась в этом этапе и не учитывается в вердикте.
...
- Holdout — только раскрытие, не входит в решение. На holdout разрыв base↔лучший профиль ЕЩЁ МЕНЬШЕ чем на val (sell +0.0045, buy +0.0037), но base на holdout уже слаб (sell 0.652, buy 0.655). Сравнение профилей на деградирующем базовом сигнале малоинформативно — holdout не подтверждает отрицательный вывод, а молчит.
...
- **Противоречие с Stage 5.0-prep.** Prep показывал `time_only` AUC=0.6286 > `no_time` (1000 фракталов) AUC=0.6113. В 5.0d `no_structure` (price+ATR+time, без 9 структурных) = 0.534 — ниже чем `time_only` из prep на 0.094. Возможные причины: разные сплиты/transforms (5.0d asinh vs prep current), или price-токены в `no_structure` шумят и ухудшают (подтверждается тем, что `no_price` чуть улучшает AUC на buy: +0.0006). Это не разобрано в этапе.
- **Природа отрицательного результата не установлена.** Вердикт `h6_off05_target_exhausted` означает, что ни один профиль не превосходит base на val 2021-2022. Но не различает: (H1) сигнал слабый/отсутствует, или (H2) сигнал затухает во времени и сравнение на одном val-окне локально. Stage 5.0f (диагностика стационарности) нужен для разделения этих гипотез.
...
Постановка H6_off05 stop broken на текущих 9 профилях исчерпана на val 2021-2022. Перед сменой target или закрытием Fractal Stop нужен Stage 5.0f — диагностика стационарности сигнала: разделить гипотезы «сигнал слабый» (H1) vs «сигнал затухает во времени» (H2). Walk-forward AUC + rolling refit + yearly heatmap на `base_raw_plus_time`, лучшем профиле 5.0d и `time_only`. Если H2 подтверждена — смена target с коротким окном переобучения может сработать. Если H1 — закрывать Fractal Stop, переход на regression_updn/triple barrier. Fractal Stop как семейство целей не закрыт в любом случае — остаются другие постановки (сторона, время до пробоя, выход, режим).
