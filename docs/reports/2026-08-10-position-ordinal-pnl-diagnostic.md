...
Обнаружен систематический паттерн: фильтр по `open_positions >= threshold` значительно улучшает результаты.

**Как работает фильтр:**

На момент появления сигнала проверяем `open_positions` (количество открытых позиций):
- Если `open_positions < threshold` — не открываем позицию (пропускаем сигнал)
- Если `open_positions >= threshold` — открываем позицию

Мы НЕ получаем прибыль/убыток от пропущенных сигналов. Результаты учитывают только прибыль от открытых сделок.

**Сравнение фильтров (threshold = минимальное количество открытых позиций для торговли):**

| Threshold | n_trades | total_profit | avg_profit | PF | WR |
|-----------|----------|--------------|------------|------|------|
...
**Детальное сравнение threshold=4 vs threshold=5:**

| Метрика | threshold=4 | threshold=5 |
|---------|-------------|-------------|
| n_trades | 1389 | 682 |
...
**Устойчивость threshold=5 по годам:**
- 2021: n=273, profit=16288, PF=3.58, WR=65%
- 2022: n=409, profit=18678, PF=2.96, WR=64%

**Устойчивость threshold=5 по кандидатам:**
- 30/30 кандидатов с profit > 0 (100%)
- 28/30 кандидатов с PF > 1.5 (93%)

**Природа паттерна:**

`open_positions >= 5` означает, что кандидат уже открыл 4+ позиции перед текущей сделкой. Это указывает на:
...
**Реализация в live-торговле:**

Фильтр реализуется просто: перед открытием позиции проверяем `open_positions` и сравниваем с threshold.

**Пример для threshold=5:**
- Появляется сигнал, `open_positions = 1` → пропускаем
- Появляется сигнал, `open_positions = 3` → пропускаем
- Появляется сигнал, `open_positions = 5` → **открываем позицию**

**Ограничения:**
- Пропускаем 92.4% сигналов (open_positions < 5)
- Очень мало сделок: 682 на 32 кандидата (~21 на кандидата)
- Входим поздно, когда тренд уже начался (4+ позиции уже открыты)

**Рекомендация:**

threshold=5 даёт максимальную прибыль (+55% vs baseline) и PF (3.20 vs 1.06), но threshold=4 более устойчив:
- Все 32 кандидата прибыльны (vs 30/30 для threshold=5)
- Меньше вариация PF по годам (2.48/1.46 vs 3.58/2.96)
- Больше сделок (1389 vs 682) для статистической значимости

Для live-торговли рекомендуется **threshold=4** как баланс прибыли и надёжности.
...
**Обнаружен систематический паттерн:** фильтр по `open_positions >= threshold` значительно улучшает результаты. 28 из 32 кандидатов показывают PF_5+ > PF_1-4 И PF_5+ > 1.5.

**Абсолютная прибыль:**
- Ordinal 1-4: total_profit=-12433, avg_profit=-1.5 (убыточны)
- Ordinal 5+: total_profit=+34966, avg_profit=+51.3 (прибыльны)
- Ordinal 5+ даёт 7.6% сделок, но 100% прибыли (компенсирует убытки ordinal 1-4)

**Фильтр `open_positions >= threshold` работает:**
- threshold=5: profit=34966 (+55% vs baseline), PF=3.20, WR=64%
- threshold=4: profit=33903 (+50% vs baseline), PF=1.78, WR=57%, более устойчив (32/32 кандидата прибыльны)

**Реализация:** Перед открытием позиции проверяем `open_positions`. Если меньше threshold — пропускаем сигнал. Если >= threshold — открываем позицию. Результаты учитывают только прибыль от открытых сделок.

**Природа паттерна:** `open_positions >= 5` означает, что кандидат уже открыл 4+ позиции перед текущей сделкой. Это указывает на селективное поведение (сильный сигнал), высокую волатильность или уверенность модели.

### Next steps

1. **Реализовать фильтр `open_positions >= threshold` в MQL5.** Добавить проверку перед открытием позиции. Рекомендуется threshold=4 как баланс прибыли и надёжности.
2. **Проверить drawdown.** Оценить максимальную просадку для threshold=4 vs threshold=5.
3. **Комбинация фильтров.** Исследовать open_positions + ATR + вероятность модели для улучшения результатов.
...
**DIAGNOSTIC_ONLY.** Результаты не являются доказательством прибыльности, готовности к торговле или качества модели. Это diagnostic analysis механики исполнения в max=64 пилоте. Фильтр `open_positions >= threshold` — гипотеза для дальнейшего исследования, не готовая торговая стратегия.
