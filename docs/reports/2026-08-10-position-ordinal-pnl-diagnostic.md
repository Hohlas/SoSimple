...
Обнаружен систематический паттерн: фильтр по ordinal > N значительно улучшает результаты.

**Сравнение фильтров:**

| N | n_trades | total_profit | avg_profit | PF | WR |
|---|----------|--------------|------------|------|------|
| 0 (baseline) | 8934 | 22533 | 2.5 | 1.06 | 45% |
| 1 | 5277 | 20499 | 3.9 | 1.10 | 47% |
| 2 | 2819 | 25305 | 9.0 | 1.25 | 50% |
| 3 | 1389 | 33903 | 24.4 | 1.78 | 57% |
| 4 | 682 | **34966** | 51.3 | 3.20 | 64% |
| 5 | 378 | 23096 | 61.1 | 3.83 | 67% |

**Детальное сравнение N=3 vs N=4:**
...
**Устойчивость N=4 по годам:**
- 2021: n=273, profit=16288, PF=3.58, WR=65%
- 2022: n=409, profit=18678, PF=2.96, WR=64%

**Устойчивость N=4 по кандидатам:**
- 30/30 кандидатов с profit > 0 (100%)
- 28/30 кандидатов с PF > 1.5 (93%)

**Природа паттерна:**

Ordinal — это `open_positions` на момент открытия (polling snapshot на H1-баре). Ordinal > 4 означает, что кандидат уже открыл 4+ позиции перед текущей сделкой. Это указывает на:
...
**Как реализовать в live-торговле:**

Для фильтра N=4:
- Считать количество открытых позиций в данный момент
- Если `open_positions < 4` — не торговать
- Если `open_positions >= 4` — торговать

**Проблема:** мы торгуем только когда уже есть 4 открытых позиции. Это означает:
- Кандидат должен "набрать" 4 позиции перед торговлей
- Мы входим поздно, когда тренд уже начался
- Пропускаем 92.4% сделок (ordinal 1-4)

**Рекомендация:**

N=4 даёт максимальную прибыль (+55% vs baseline) и PF (3.20 vs 1.06), но N=3 более устойчив:
- Все 32 кандидата прибыльны (vs 30/30 для N=4)
- Меньше вариация PF по годам (2.48/1.46 vs 3.58/2.96)
- Больше сделок (1389 vs 682) для статистической значимости

Для live-торговли рекомендуется **N=3** как баланс прибыли и надёжности.
...
**Обнаружен систематический паттерн:** фильтр по ordinal > N значительно улучшает результаты. 28 из 32 кандидатов показывают PF_5+ > PF_1-4 И PF_5+ > 1.5.
...
**Фильтр ordinal N+ работает:**
- N=4: profit=34966 (+55% vs baseline), PF=3.20, WR=64%
- N=3: profit=33903 (+50% vs baseline), PF=1.78, WR=57%, более устойчив (32/32 кандидата прибыльны)

**Природа паттерна:** Ordinal > 4 означает, что кандидат уже открыл 4+ позиции перед текущей сделкой. Это указывает на селективное поведение (сильный сигнал), высокую волатильность или уверенность модели.

### Next steps

1. **Реализовать фильтр ordinal N+ в MQL5.** Добавить проверку `open_positions > N` перед открытием позиции. Рекомендуется N=3 как баланс прибыли и надёжности.
2. **Проверить drawdown.** Оценить максимальную просадку для фильтра N=3 vs N=4.
3. **Комбинация фильтров.** Исследовать ordinal + ATR + вероятность модели для улучшения результатов.
...
**DIAGNOSTIC_ONLY.** Результаты не являются доказательством прибыльности, готовности к торговле или качества модели. Это diagnostic analysis механики исполнения в max=64 пилоте. Фильтр ordinal N+ — гипотеза для дальнейшего исследования, не готовая торговая стратегия.
