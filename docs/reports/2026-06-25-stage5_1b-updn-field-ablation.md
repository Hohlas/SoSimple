...
`impulse` снова остался `mixed_or_unclear`. Сравнение со Stage 5.1 через add-delta выглядит как рост, но это частично артефакт деградации baseline:

| Цель | 5.1 drop val | 5.1 add val | 5.1b drop val | 5.1b add val | 5.1 `add_impulse` AUC | 5.1b `add_impulse` AUC | абсолютный рост |
|---|---:|---:|---:|---:|---:|---:|---:|
| sell | -0.0036 | +0.0164 | -0.0009 | +0.0263 | 0.6514 | 0.6542 | +0.0029 |
| buy | -0.0010 | +0.0200 | -0.0004 | +0.0338 | 0.6612 | 0.6672 | +0.0060 |

Рост add-delta (sell +0.0099, buy +0.0138) в основном объясняется падением baseline: `clock_shift` слабее `time_only` на -0.0092/-0.0084. Абсолютный AUC `add_impulse` на sell почти не изменился (+0.0029). На buy сдвиг чуть больше (+0.0060), но без CI это может быть шум.

Правильный тест на поглощение: сравнить абсолютный AUC `time_only + impulse` (5.1) и `clock_shift + impulse` (5.1b). Если `shift` перекрывается с `impulse`, добавление `shift` к `impulse` не должно помогать. Фактически shift добавляет небольшую величину сверх impulse (sell +0.0029, buy +0.0060), но без CI это ненадёжно. Поэтому вывод «shift не поглотил сигнал impulse» не подтверждён — данные не исключают частичное перекрытие.

Drop-one роль `impulse` внутри `structure_full` ослабла: seed-согласованность упала с 3/3 до 2/3 на обеих целях, delta уменьшилась (sell -0.0036 → -0.0009). Это означает, что в присутствии `shift` структура лучше компенсирует отсутствие `impulse`. Поэтому `impulse` полезен как часть диагностического `back_impulse_combo`, но не получает самостоятельный useful verdict.
