...
Oracle проходит gate: если знать истинное время до пробоя, торговая логика имеет положительный потолок.

| Target | oracle_time_pf | trades | trades/year | yearly PF |
|---|---:|---:|---:|---|
| sell | `1.6520` | `2206` | `1103.0` | 2021 `1.6352`, 2022 `1.6682` |
| buy | `1.7244` | `1997` | `998.5` | 2021 `1.6734`, 2022 `1.7723` |

Это важное разделение: идея "время до пробоя может быть полезным" не умерла на oracle-уровне, но текущая модельная реализация не смогла приблизиться к этому oracle.

### Model Results

Все профили дали одинаковую итоговую картину:

| Target | selected profile | val Spearman | val MAE | val AUC `true>=4` | holdout Spearman | holdout MAE | holdout AUC |
|---|---|---:|---:|---:|---:|---:|---:|
...
Основной вывод: Stage 5.2 не переоткрывает ветку `H6_off05`. В текущей реализации XGBoost-регрессия времени до пробоя не извлекла полезное ранжирование: Spearman равен `0`, AUC равен `0.5`, ошибка выше constant baseline.

Oracle-preflight при этом положительный. Значит, сама торговая идея "различать быстро пробиваемые и долго живущие уровни" теоретически имеет ценность, но текущие признаки и модель не научились предсказывать это время.

Нельзя делать вывод, что время до пробоя бесполезно вообще. Можно делать более узкий вывод: текущий набор Stage 5.2 профилей на `H6/off=0.5` не даёт рабочей модели времени до пробоя.

## Limitations / Open Questions

- Все model gate checks провалены на обеих целях.
- Все профили дали одинаковые итоговые метрики; это требует отдельной диагностики распределения предсказаний. Возможные причины: модель схлопывается в почти постоянное предсказание, цель слишком цензурирована для обычной регрессии, либо текущие признаки не несут информации о порядке пробоя.
...
- Сначала провести post-mortem Stage 5.2: сохранить распределения предсказаний по профилям и seed, проверить, действительно ли модель выдаёт почти константу.
