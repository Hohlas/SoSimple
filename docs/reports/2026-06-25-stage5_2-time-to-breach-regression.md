...
- `auc_ge_0_70`: PASS на обеих целях; sell `0.7005` — на грани порога, в пределах шума
...
Единственная провальная проверка — MAE vs constant. Это известная проблема censored-регрессии (61-63% строк имеют target = 7, константа 7 получает MAE = 0 на них), а не слабость признаков: Spearman и AUC показывают, что ранжирование работает.
...
Дополнительная оговорка по JSON: `oracle_preflight[target].pass = True` внутри секции `oracle_preflight` означает только, что `oracle_time_pf >= 1.3` по абсолютной величине. Он не учитывает невалидность сравнения с binary-oracle. Валидный oracle-gate находится в `gate_results[target].oracle_gate.pass = False` с `reason = "invalid_oracle_binary_comparison"`.
...
Главный отрицательный результат: обычная регрессия одного числа `bars_to_breach` плохо согласуется с censored target. Constant baseline `7` выигрывает по MAE, потому что 61-63% строк реально не пробиваются за `H=6` и имеют target = 7. Это единственная причина провала model gate: 7 из 8 проверок прошли, включая Spearman, AUC, MAE ≤ 3 и годовую согласованность. Проблема в метрике MAE, не в признаках.
...
Итог: идея времени до пробоя показала рабочий ранжирующий сигнал (Spearman 0.31-0.33, AUC 0.70-0.71), но обычная регрессия с MAE-gate — неправильная постановка для censored target. Следующий шаг должен использовать дискретную/цензурированную постановку, где MAE заменён на метрики, устойчивые к цензуре.
