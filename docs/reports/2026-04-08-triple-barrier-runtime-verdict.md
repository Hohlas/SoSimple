# Triple Barrier: текущая схема подтверждена на свежем MT4-прогоне

> **Date**: 2026-04-08 17:22 MSK
> **Status**: Completed
> **Goal**: Проверить текущую TB-схему на свежем прогоне тестера и зафиксировать, насколько MT4 согласуется с Python
...
После полной пересборки TB понадобилась свежая проверка в MT4. На этом этапе сравнивалась уже текущая схема:

- TB-разметка стартует от времени строки сигнала;
- offline-правило уже зафиксировано на validation;
- MT4 и tester используют один и тот же `ml_signals_tb.csv`.

Задача этапа была простой: посмотреть, насколько MT4 и Python совпадают по самой сути сделки, если обе стороны работают от одной и той же актуальной базы.

## What Was Done

- В `processing/label_signals.py` закреплена привязка времени: TB-разметка считает исход от времени строки сигнала.
...
Свежий MT4-прогон подтверждает, что текущая TB-схема в целом согласуется с Python:

- уровни SL/TP между Python и MT4 совпадают почти идеально;
- по жёстким исходам `TP/SL` совпадение высокое;
- MT4 дал `PF=1.27`, что близко к результату вне MT4 `PF=1.11`.

Оставшаяся разница объясняется в основном уже не уровнями и не разметкой, а самими правилами торговли в MT4:

- при открытой позиции новый сигнал пропускается (`PosBlock`);
- часть сделок закрывается по лимиту времени (`HoldOverTime`);
- часть сделок закрывается по новому обратному сигналу (`TB_Reversal`);
- вход происходит на следующем баре.

То есть на этом этапе TB можно считать согласованным с MT4 по основным торговым элементам. Следующий шаг нужен уже не для “спасения” TB, а для ещё более точного сравнения: считать результат вне MT4 по тем же правилам торговли, что и в MT4.
