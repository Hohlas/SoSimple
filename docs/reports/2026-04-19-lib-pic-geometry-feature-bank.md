...
Модуль не использует `Up/Dn`, потому что этот этап изолирует именно геометрию уровня. `Up/Dn` — отдельный тип информации: историческая реакция цены на уровень за фиксированные горизонты.
...
`Up/Dn` не считаются заглядыванием вперёд сами по себе.

Они формируются в `lib_PIC.mqh` инкрементально: уровень появился, затем по уже прошедшим барам накапливается максимум движения вверх/вниз от цены этого уровня. В строку `Nero.csv` попадает состояние, известное на момент записи строки.

Поэтому причина исключения `Up/Dn` из этого модуля другая:

- не смешивать форму уровня с последующей реакцией цены;
- получить чистый geometry-bank по `front/back/reverse`;
- проверить `Up/Dn` позже отдельным path-reaction feature-bank.
