...
- `Up12/Dn12/.../Up6/Dn6`, потому что это не геометрия уровня, а отдельная информация о реакции цены после уровня;
- новые поля из `lib_PIC`, которых пока нет в `Nero.csv`.

Важно: `Up/Dn` в текущем pipeline не считаются заглядыванием вперёд, если они накоплены только по барам, уже известным на момент строки. Они исключены здесь только для чистоты geometry-слоя.
