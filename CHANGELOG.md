...
### Вывод
❌ FAIL — торговая постановка breach+fav на RF не работает: PF 0.6–0.98, gap 10–30× до oracle. Причина: слабая модель, не механика (oracle PF=∞, 0 убыточных лет). Рекомендация: Stage 3 — улучшение breach-классификатора (XGBoost/Transformer, новые признаки).
