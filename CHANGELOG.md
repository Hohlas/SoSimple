...
### Добавлено
- **`statistics/signal_tracer.py` v2.0**: инструмент разбора полётов — сравнивает ML-предсказание с реальным ходом цены по конкретному фракталу.
  - **`--time "YYYY.MM.DD HH:MM"`**: полное досье одного сигнала.
  - **`--batch [--top N] [--min-ratio X]`**: автоматический поиск top-N высокорейтинговых сигналов с ratio ≥ порога, сводная таблица + детальные досье + CSV экспорт (`--csv-out`).
- **Точная реплика формулы MT4 SL/TP** (lib_ML_Signal.mqh:171–193): `SL = max(pred * ScaleK * ATR, ATR * Min_SL_ATR)`, `TP = SL * min(ratio / MinRatio, MaxRR)`. Предыдущая формула `1.6 * ATR` была полностью неверной — не использовала ML-предсказания.
- **INI-парсер**: параметры ML читаются из `$o$imple.ini` (ML_MinRatio=3.5, ML_ScaleK=20, ML_Min_SL_ATR=2.0) вместо захардкоженных значений.
- **4-категорийная классификация исхода**: `TP_CLEAR` / `SL_CLEAR` / `BOTH_HIT` / `TIMEOUT`. Категория `BOTH_HIT` (оба барьера достигнуты, порядок неизвестен) — ключ к MFE/MAE иллюзии.
- **Lag Bias детектор**: вычисляет задержку между формированием фрактала (f0[0]) и сигнальным баром; классифицирует как нормальную задержку / умеренную / устаревший фрактал.

### Результаты (batch, top-10, ratio ≥ 4.0, ml_signals.csv 2004–2026)
...
| 3 | 2010.08.06 11:00 | BUY | 258.19 | 2.00 | 8.00 | 0.00 | 29.10 | SL_CLEAR | 237h |
| 4 | 2021.08.27 18:00 | BUY | 186.40 | 2.00 | 8.00 | 0.00 | 39.70 | SL_CLEAR | 8975h |
| 5 | 2013.03.11 13:00 | BUY | 184.28 | 2.00 | 8.00 | 5.30 | 0.00 | TIMEOUT | 85h |
| 6 | 2018.10.17 07:00 | BUY | 149.28 | 2.00 | 8.00 | 2.20 | 6.70 | SL_CLEAR | 17h |
| 7 | 2017.10.04 09:00 | BUY | 142.60 | 2.00 | 8.00 | 0.30 | 4.80 | TIMEOUT | 285h |
| 8 | 2006.04.07 07:00 | BUY | 132.62 | 2.00 | 8.00 | 0.00 | 6.50 | SL_CLEAR | 37h |
| 9 | 2022.09.16 17:00 | BUY | 126.54 | 2.00 | 8.00 | 6.20 | 25.70 | SL_CLEAR | 30h |

Итого по 9 сигналам: **TP_CLEAR=0, SL_CLEAR=7, BOTH_HIT=0, TIMEOUT=2**. Средний lag фрактала: **1308 баров**.

### Вывод
1. **Высокий ratio ≠ надёжный сигнал**: ни один из 9 высокорейтинговых сигналов (ratio 126–585) не достиг TP. Все сигналы — только BUY, что указывает на перекос модели.
2. **SL всегда от пола (Min_SL_ATR)**: компонент `pred_dn * ScaleK * ATR` ≪ `ATR * 2.0` — модель предсказывает dn_12 близко к нулю (ratio → ∞), но реальный ход вниз всегда превышает SL. Гипотеза MFE/MAE иллюзии подтверждается на всех примерах.
3. **Lag bias критичен**: средний фрактал в момент сигнала устарел на 1308 баров (~54 дня). Модель оценивает устаревший контекст, игнорируя более свежие ценовые события. Это может быть причиной патологически высоких ratio — мёртвые фракталы не обновляются.
4. **Следующий шаг**: провести `--batch` на реальных сделках MT4-тестера (iSignal=3, убыточные) с точными временами открытия для прямого сопоставления с журналом стратегии.
