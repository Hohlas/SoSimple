...
- **summary**: Added pruning runner for the 11 passed fixed rules. The run produced `pruning_passed` with retained subset manifest and pairwise overlap/correlation artifacts.
...
- **decision**: Retain 5 rules and drop 6 direct strong duplicates; representatives inside duplicate groups are selected by `BS_p05 / max_drawdown_r`.
