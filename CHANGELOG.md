...
> **Предупреждение**: Читай только первые 150 строк этого файла.
...
- **Асимметричный R:R**: Вместо статического R:R>=1.0, внедрен расчет Take Profit пропорционально уверенности модели: `TP = SL * min(Ratio / ML_MinRatio, ML_MaxRR)`.
...
- *Открытие*: Чем жестче шаг трала (`0.5 ATR` vs `1.5 ATR`), тем выше оборачиваемость и PF (0.96 vs 0.91), так как цена почти всегда откатывается перед 12-м часом, а трал успевает "прихватить" пик (MFE) задолго до окончания таймаута.

### Вывод
Отсутствие фиксации локальных выбросов (MFE) приводило к иллюзии прибыльности в скрипте Python (Look-Ahead Bias) и потере реальной прибыли в MT4 (HoldOverTime). Использование жесткого математического Trailing Stop, динамически подтягивающего SL, успешно симулирует "взятие пиковых вершин" из оценки Python. Следующий логичный шаг — использование Генетической оптимизации Strategy Tester для подбора самых ликвидных дистанций остановки (`ML_Min_SL_ATR`, `ML_Trl_Start_ATR`, `ML_Trl_Step_ATR`).
