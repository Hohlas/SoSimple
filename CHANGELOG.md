...
## [2026-03-21] — ME-12: Отладка ML_TRADE() в MT4 Strategy Tester (XAUUSD H1)

### Исправлено
- **Одновременные противоположные позиции**: ML_TRADE() открывал BUY поверх открытого SELL и наоборот. Исправлено добавлением `SEL.Typ == NONE` в условие BUY и `BUY.Typ == NONE` в условие SELL.

### Добавлено
- **`ML_MinRatio`**: Фильтр сигналов по минимальному значению `ratio_up`/`ratio_dn`. Глобальная переменная в `lib_ML_Signal.mqh`, по умолчанию 0.0 (без фильтра).

### Результаты (XAUUSD H1, 2023, депозит $10k, Stp=Prf=3, R:R=1:1)
