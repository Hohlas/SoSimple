...
bool INPUT_FILE_READ (){// считывание из csv файла входных параметров
   ...
   while (!FileIsEnding(File)){ 
      ...
      if (!GlobalVariableCheck(EXP[e].Name+EXP[e].Sym+S0(EXP[e].Per)))     GlobalVariableSet(EXP[e].Name+EXP[e].Sym+S0(EXP[e].Per), iTime(EXP[e].Sym,(ENUM_TIMEFRAMES)EXP[e].Per,0));  // глобал чарта эксперта для проверки готовности в ф. END()
...
void MAIL_SEND(){ // отправляем мыло из файла Reports.csv с отчетами
   ...
   for(int i=0; i<OrdersHistoryTotal(); i++){// перебераем историю сделок эксперта
      ...
      if (OrderType()==6 && iTime(NULL,(ENUM_TIMEFRAMES)60,0)-OrderOpenTime()<3300)   Roll+=profit;// 6-ролловер, т.е. инвестиции. За прошлый час с небольшим запасом в 55мин = 3300с 
   ...
   if (AccountInfoDouble(ACCOUNT_PROFIT)>0) CurPrf="+"+S1(AccountInfoDouble(ACCOUNT_PROFIT)*100/AccountBalance())+"%"; // текущая незакрытая прибыль в процентах
   if (AccountInfoDouble(ACCOUNT_PROFIT)<0) CurPrf=    S1(AccountInfoDouble(ACCOUNT_PROFIT)*100/AccountBalance())+"%";
