...
void RefreshPriceArrays(int count=0) { // count<=0 — копировать всю историю; частичная копия ломает доступ по индексам iLowest/iHighest > count
   ...
   if (count <= 0 || count > bars) count = bars;
...
void RefreshRates() { RefreshPriceArrays(); }
