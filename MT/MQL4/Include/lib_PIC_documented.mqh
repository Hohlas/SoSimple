...
bool USE_NORMALIZED_OUTPUT =
    true; // true - нормализованные значения, false - сырые значения
//====================================================================
...
void EXPERT::NERO_CSV_CREATE(int cur_bar) {
  ...
  float frontValues[LevelsAmount];
  ...
  float percentileFront =
      CALCULATE_PERCENTILE(frontValues, frontCount, NORMALIZATION_PERCENTILE);
  float percentilePwrSum =
      CALCULATE_PERCENTILE(pwrSumValues, pwrSumCount, NORMALIZATION_PERCENTILE);
  float percentileRev =
      CALCULATE_PERCENTILE(revValues, revCount, NORMALIZATION_PERCENTILE);
  float percentileImp =
      CALCULATE_PERCENTILE(impValues, impCount, NORMALIZATION_PERCENTILE);
  float percentileCnt =
      CALCULATE_PERCENTILE(cntValues, cntCount, NORMALIZATION_PERCENTILE);

  for (uchar f = 1; f < LevelsAmount; f++) {
    if (F[f].P == 0)
      continue;

    normFrnt = hasData ? (float)PiecewiseNormalize(F[f].FrntVal, minFront,
                                                   percentileFront, maxFront)
                       : 0.5f;
    normBack = hasData ? (float)PiecewiseNormalize(F[f].BackVal, minFront,
                                                   percentileFront, maxFront)
                       : 0.5f;

    normPwrSum = hasData
                     ? (float)PiecewiseNormalize(F[f].PwrSum, minPwrSum,
                                                 percentilePwrSum, maxPwrSum)
                     : 0.5f;
    normRev = hasData ? (float)PiecewiseNormalize(F[f].Rev, minRev,
                                                  percentileRev, maxRev)
                      : 0.5f;
    normImp = hasData ? (float)PiecewiseNormalize(F[f].Imp, minImp,
                                                  percentileImp, maxImp)
                      : 0.5f;
    normCnt = hasData ? (float)PiecewiseNormalize(F[f].Cnt, minCnt,
                                                  percentileCnt, maxCnt)
                      : 0.5f;
    ...
    if (USE_NORMALIZED_OUTPUT) {
      NeroInfo = NeroInfo + ";" + S0(F[f].T) + ":" + S_NORM(normPrice) + ":" +
                 S0(F[f].Dir) + ":" + S_NORM(normFrnt) + ":" +
                 S_NORM(normBack) + ":" + S0(F[f].Strong) + ":" +
                 S_NORM(normBrk) + ":" + S_NORM(normRev) + ":" +
                 S_NORM(normPwrSum) + ":" + S_NORM(normCnt) + ":" +
                 S_NORM(normImp);
    } else {
      NeroInfo = NeroInfo + ";" + S0(F[f].T) + ":" + S4(F[f].P) + ":" +
                 S0(F[f].Dir) + ":" + S4(F[f].FrntVal) + ":" +
                 S4(F[f].BackVal) + ":" + S0(F[f].Strong) + ":" + S0(F[f].Brk) +
                 ":" + S4(F[f].Rev) + ":" + S4(F[f].PwrSum) + ":" +
                 S0(F[f].Cnt) + ":" + S4(F[f].Imp);
    }
    cnt++;
  }
