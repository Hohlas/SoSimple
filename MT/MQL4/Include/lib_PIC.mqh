...
//!   - Инкрементальное накопление Up[5]/Dn[5] для каждого фрактала (горизонты H12/H24/H48/H3/H6)
//!   - Экспорт данных в CSV: 23 поля на фрактал; строка-ATR = Atr.Slow
//!
//! Архитектура:
...
//!
//! @note Текущий файл в репозитории хранится как UTF-8 с CRLF
   ...
   for (uchar f = 1; f < LevelsAmount; f++) {
      ...
      if (USE_NORMALIZED_OUTPUT) {
         ...
         NeroInfo = NeroInfo + ";" +
                    ...
                    S4(F[f].Up[H6])  + ":" + S4(F[f].Dn[H6])  + ":" + S4(F[f].Atr) + ":" + S0(SHIFT(F[f].T) - cur_bar);
      } else {
         NeroInfo = NeroInfo + ";" +
                    S0(F[f].T) + ":" +
                    S4(F[f].P) + ":" +
